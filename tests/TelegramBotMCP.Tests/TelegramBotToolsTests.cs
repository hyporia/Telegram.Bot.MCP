...
public class TelegramBotToolsTests : IAsyncLifetime
{
    ...
    [Theory, AutoData]
    public async Task GetUnreadMessages_WithMessages_ReturnsFormattedJson(UpdateDTO[] updates)
    {
        // Arrange
        _botMock.Setup(m => m.GetUpdates(It.IsAny<int>())).ReturnsAsync(updates);

        // Act
        var result = await _tools.GetNewMessages();
    ...
    [Fact]
    public async Task GetUnreadMessages_NoMessages_ReturnsEmptyArray()
    {
        // Arrange
    ...
    [Fact]
    public async Task GetUnreadMessages_ExceptionThrown_ReturnsErrorMessage()
    {
        // Arrange
